...
structure MontConstants where
  ...
  R_mod_N_eq : R_mod_N = (cfg.R : ZMod cfg.N)  -- Ensures R_mod_N is the canonical R mod N
...
theorem asymptotic_savings :
    ...
    -- Note: For n ≥ 2, persistent_conversions/traditional_conversions ≤ 1
    ∀ n : ℕ, n ≥ 2 →
      (persistent_conversions n : ℚ) / traditional_conversions n ≤ 1 := by
  intro n hn
  simp only [persistent_conversions, traditional_conversions]
  -- We need to show: (n + 2 : ℚ) / (2 * n) ≤ 1
  -- Equivalently: n + 2 ≤ 2 * n, i.e., 2 ≤ n (which we have)
  have h_denom_pos : (0 : ℚ) < 2 * n := by positivity
  rw [div_le_one h_denom_pos]
  -- Now showing: (n + 2 : ℚ) ≤ 2 * n
  have h_nat : n + 2 ≤ 2 * n := by omega
...
theorem equality_in_montgomery (const : MontConstants cfg) (a b : ZMod cfg.N) :
  ...
  · intro h
    ...
    -- Since R is coprime to N (by cfg.R_coprime), R mod N is a unit in ZMod N
    -- Use the R_mod_N_eq field to connect R_mod_N to cfg.R
    have hR_unit : IsUnit const.R_mod_N := by
      rw [const.R_mod_N_eq]
      -- (cfg.R : ZMod cfg.N) is a unit iff gcd(cfg.R, cfg.N) = 1
      apply ZMod.isUnit_of_coprime
      exact cfg.R_coprime
    -- Cancel the unit from both sides: a * R = b * R implies a = b
    exact mul_right_cancel₀ (IsUnit.ne_zero hR_unit) h1
  · intro h
    simp [h]
...
/--
  ...
  ✓ Theorem: equality_in_montgomery (unit cancellation via R_coprime)
  ✓ Theorem: asymptotic_savings (for n ≥ 2, ratio ≤ 1)
...
#check QMNF.PersistentMontgomery.equality_in_montgomery
#check QMNF.PersistentMontgomery.asymptotic_savings
