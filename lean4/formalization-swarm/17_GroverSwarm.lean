...
/--
  ...
  For N ≥ 16, Nat.sqrt N ≥ 4, and (s * 785) / 1000 + 1 ≤ s for s ≥ 4:
  - s = 4: (4 * 785) / 1000 + 1 = 3 + 1 = 4 ≤ 4 ✓
  - s ≥ 5: 0.785s + 1 < s when s > 4.65

  This bound is verified computationally for all s ≥ 4.
-/
theorem optimal_sqrt_scaling (N : ℕ) (hN : N ≥ 16) :
    optimalIterations N ≤ Nat.sqrt N := by
  unfold optimalIterations
  -- For N ≥ 16, sqrt N ≥ 4, and for s ≥ 4: (s * 785) / 1000 + 1 ≤ s
  have hsqrt_ge_4 : Nat.sqrt N ≥ 4 := by
    calc Nat.sqrt N ≥ Nat.sqrt 16 := Nat.sqrt_le_sqrt hN
      _ = 4 := by native_decide
  -- Key lemma: for s ≥ 4, (s * 785) / 1000 + 1 ≤ s
  have h_key : ∀ s : ℕ, s ≥ 4 → (s * 785) / 1000 + 1 ≤ s := by
    intro s hs
    -- s * 785 / 1000 < s since 785 < 1000
    have hdiv : s * 785 / 1000 < s := by
      apply Nat.div_lt_self
      · omega  -- s * 785 > 0 since s ≥ 4
      · omega  -- 1000 > 1
    -- For s ≥ 4: s * 785 / 1000 ≤ s - 1, so adding 1 gives ≤ s
    omega
  exact h_key (Nat.sqrt N) hsqrt_ge_4

/--
  Quadratic speedup: O(√N) vs O(N) classical search

  MATHEMATICAL JUSTIFICATION:
  For N ≥ 2, we show optimalIterations N < N.
  Key insight: sqrt N + 1 ≤ N for N ≥ 2, and
  (sqrt N * 785) / 1000 + 1 ≤ sqrt N + 1.

  Note: The original theorem had N > 0, but optimalIterations 1 = 1,
  and 1 < 1 is false. Hence we require N ≥ 2.
-/
theorem quadratic_speedup (N : ℕ) (hN : N ≥ 2) :
    optimalIterations N < N := by
  unfold optimalIterations
  -- Goal: (Nat.sqrt N * 785) / 1000 + 1 < N
  -- Strategy: (sqrt N * 785) / 1000 + 1 ≤ sqrt N + 1 ≤ N
  have h1 : Nat.sqrt N * 785 / 1000 + 1 ≤ Nat.sqrt N + 1 := by
    have hdiv : Nat.sqrt N * 785 / 1000 ≤ Nat.sqrt N := Nat.div_le_self _ _
    omega
  have h2 : Nat.sqrt N + 1 ≤ N := by
    -- For N ∈ {2, 3}: check directly
    -- For N ≥ 4: sqrt N ≤ N - 1 since (sqrt N)² ≤ N and sqrt N ≥ 2
    rcases Nat.lt_or_ge N 4 with hN4 | hN4
    · -- N ∈ {2, 3}
      interval_cases N <;> native_decide
    · -- N ≥ 4
      have hsq : Nat.sqrt N * Nat.sqrt N ≤ N := Nat.sq_sqrt_le N
      have hsqrt_ge_2 : Nat.sqrt N ≥ 2 := by
        calc Nat.sqrt N ≥ Nat.sqrt 4 := Nat.sqrt_le_sqrt hN4
          _ = 2 := by native_decide
      -- With sqrt N ≥ 2 and (sqrt N)² ≤ N, we get sqrt N ≤ N - sqrt N ≤ N - 2
      -- Hence sqrt N + 1 ≤ N - 1 < N
      nlinarith [Nat.sq_sqrt_le N, hsqrt_ge_2]
  omega
