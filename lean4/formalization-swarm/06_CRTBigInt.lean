...
theorem fibonacci_coprime (n : ℕ) :
  ...
  induction n with
  | zero =>
    -- Base case: gcd(21, 34) = 1
    -- 21 = 3 × 7, 34 = 2 × 17, no common factors
    decide
  | succ n ih =>
    -- fib(n+2) = fib(n) + fib(n+1)
    -- gcd(fib(n+1), fib(n+2)) = gcd(fib(n+1), fib(n))
    have h_rec : fibonacciModulus (n + 2) = fibonacciModulus n + fibonacciModulus (n + 1) := rfl
    rw [Nat.Coprime]
    rw [h_rec]
    -- Use: gcd(a, a+b) = gcd(a, b) for the key step
