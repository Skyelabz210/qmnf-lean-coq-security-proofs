...
def inMemoryMatMul (arr_A arr_B : RayRamArray cfg) (N : ℕ)
    (hN : N * N ≤ cfg.cell_count) (hN_pos : N > 0) : RayRamArray cfg :=
  -- Each cell computes its element of C = A × B
  -- C[i,j] = Σ_k A[i,k] × B[k,j]
  ⟨fun idx =>
    let i := idx.val / N
    let j := idx.val % N
    if hidx : idx.val < N * N then
      -- i = idx / N < N*N / N = N (when N > 0)
      have hi : i < N := Nat.div_lt_of_lt_mul hidx
      -- j = idx % N < N (when N > 0)
      have hj : j < N := Nat.mod_lt idx.val hN_pos
      let dot_product := Finset.range N |>.sum (fun k =>
        if hk : k < N then
          let a_ik := matrixCell cfg arr_A N ⟨i, hi⟩ ⟨k, hk⟩ hN
          let b_kj := matrixCell cfg arr_B N ⟨k, hk⟩ ⟨j, hj⟩ hN
          a_ik * b_kj
        else 0)
      ⟨dot_product, 0, 0⟩
    else
